...
Statistics launchBaseParallelGpuSolver(const board::Board & board)
{
    Statistics retVal(solversEnabled[PARALLEL_GPU_BASE]);
    ...
    if (solversEnabled[PARALLEL_GPU_BASE] == true)
    {
        Timer time;
        Timer timeInit;
...
Statistics launchIncrementalStackParallelGpuSolver(const board::Board & board)
{
    Statistics retVal(solversEnabled[PARALLEL_GPU_INCREMENTAL]);
    auto pgMilliseconds = std::numeric_limits<double>::quiet_NaN();
    ...
    if (solversEnabled[PARALLEL_GPU_INCREMENTAL] == true)
    {
        Timer time;
        Timer timeInit;
        Timer timeGeneration;
        ...
        if (cuda::solver::verifyAllocation(d_solvers, d_outputBoards, d_outputBoardsSize))
        {
            ...
            if (cudaStatus != cudaSuccess)
            {
                printf("parallelSolvingIncrementalStack launch failed: %s\n", cudaGetErrorString(cudaStatus));
            }
            else
            {
                // cudaDeviceSynchronize waits for the kernel to finish, and returns
...
Statistics launchAOSStackParallelGpuSolver(const board::Board & board)
{
    Statistics retVal(solversEnabled[PARALLEL_GPU_AOS]);
    auto pgMilliseconds = std::numeric_limits<double>::quiet_NaN();
    ...
    if (solversEnabled[PARALLEL_GPU_AOS] == true)
    {
        Timer time;
        Timer timeInit;
        Timer timeGeneration;
...
Statistics launchSOAStackParallelGpuSolver(const board::Board & board)
{
    Statistics retVal(solversEnabled[PARALLEL_GPU_SOA]);
    auto pgMilliseconds = std::numeric_limits<double>::quiet_NaN();
    ...
    if (solversEnabled[PARALLEL_GPU_SOA] == true)
    {
        Timer time;
        Timer timeInit;
        Timer timeGeneration;
